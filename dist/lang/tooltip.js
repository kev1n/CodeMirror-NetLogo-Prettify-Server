...
export function getTooltip(view, from, to, editor) {
    ...
    if (closestTerm == '~BreedReporter' || closestTerm == '~BreedCommand') {
        ...
        if (match.Valid && match.Plural) {
            if (match.Prototype && Dictionary.Check(match.Prototype))
                closestTerm = match.Prototype;
        ...
        syntaxTree(state)
            ...
            if (node.name == 'BreedsOwn') {
                ...
                node.node.getChildren('Own').map((subnode) => {
                    if (getCodeName(state, subnode) == secondTerm + '-own')
                        correctNode = true;
                });
                if (correctNode) {
