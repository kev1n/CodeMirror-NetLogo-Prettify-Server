...
import { checkValidIdentifier, getCheckContext, checkUndefinedBreed, checkUnrecognizedWithSuggestions, } from '../utils/check-identifier';
import { getCodeName } from '../utils/code';
import { Localized } from 'src/editor';
// IdentifierLinter: Checks anything labelled 'Identifier'
export const IdentifierLinter = (view, preprocessContext, lintContext) => {
    const diagnostics = [];
    ...
    syntaxTree(view.state)
        ...
        if ((noderef.name == 'Identifier' && parent?.name != '⚠') ||
            ...
            if (checkUnrecognizedWithSuggestions(diagnostics, view, noderef.node))
                return;
