...
export const ArgumentLinter = (view, preprocessContext, lintContext) => {
    const diagnostics = [];
...
export const getArgs = function (Node) {
...
export const checkValidNumArgs = function (state, args, preprocessContext) {
    ...
    if (name.includes('Special')) {
        switch (name) {
            ...
            case 'SpecialCommandCreateLink':
                ...
                break;
            default:
                let numArgs = preprocessContext.Commands.get(func) ?? preprocessContext.Reporters.get(func) ?? getBreedProcedureArgs(name);
