...
export const getCheckContext = function (view, lintContext, preprocessContext) {
...
export const checkValidIdentifier = function (Node, value, context) {
    value = value.toLowerCase();
    // checks if parent is in a category that is always valid (e.g. 'Globals')
    if (acceptableIdentifiers.includes(Node.parent?.name ?? ''))
        return true;
    // checks if identifier is a global variable
    if (context.parseState.Globals.has(value) ||
...
export const getLocalVariables = function (Node, State, parseState) {
    let procedureVars = [];
    let procedure = null;
...
export function checkUndefinedBreed(diagnostics, context, view, noderef) {
    ...
    if (!info.Valid && info.Plural) {
        if (!checkUnrecognizedWithSuggestions(diagnostics, view, noderef)) {
            ...
            if (typeof info.Type !== 'undefined')
                addBreedAction(diagnostic, info.Type, info.Plural, info.Singular);
...
export const UnrecognizedSuggestions = {
    else: 'ifelse',
    '%': 'mod',
    '{': '[',
    '}': ']',
...
export function checkUnrecognizedWithSuggestions(diagnostics, view, node) {
    var value = getCodeName(view.state, node);
    if (UnrecognizedSuggestions.hasOwnProperty(value)) {
        let diagnostic = getDiagnostic(view, node, 'Unrecognized identifier with replacement _', 'error', value, UnrecognizedSuggestions[value]);
