import { BuildSnapshot, IntegrateSnapshot } from './code-snapshot';
...
export async function FixGeneratedCode(Editor, Source, Parent, try_again = true) {
    Source = Source.trim();
    if (Source == '')
        return Source;
    ...
    if (did_actions && try_again)
        await FixGeneratedCode(Editor, Editor.CodeMirror.state.doc.toString(), Parent, false);
    // Fifth pass: re-introduce the snapshot
    IntegrateSnapshot(Editor, Snapshot);
    if (Parent)
        IntegrateSnapshot(Editor, Parent);
...
function FixBreed(node, state, breeds) {
    ...
    let invalid_sing = ['turtle', 'link', 'patch', ...breeds].includes(state.sliceDoc(singular?.from, singular?.to).toLowerCase()) ||
        state.sliceDoc(singular?.from, singular?.to).toLowerCase() ==
            state.sliceDoc(plural?.from, plural?.to).toLowerCase();
    let invalid_plur = ['turtles', 'links', 'patches', ...breeds].includes(state.sliceDoc(plural?.from, plural?.to).toLowerCase());
