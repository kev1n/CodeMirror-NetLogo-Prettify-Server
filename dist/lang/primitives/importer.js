...
// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-ignore
import { NetLogoType, AgentContexts } from '../classes/structures.js';
// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-ignore
...
class PrimitiveImporter {
    ...
    Import() {
        // Core ones
        for (var Primitive of Reporters)
            this.ImportNL('', Primitive);
        for (var Primitive of Commands)
            this.ImportNL('', Primitive);
        // Extensions ones
        ...
        File.readdirSync(Extensions).forEach((Path) => {
            ...
            const Metadata = JSON.parse(File.readFileSync(path.join(Extensions, Path), 'utf8'));
            Metadata.prims.forEach((Primitive) => this.ImportNLW(Extension, Primitive));
        });
    }
    /** Export: Export all primitives to a file. */
    Export() {
        File.writeFileSync('./src/lang/primitives/dataset.ts', `import { Primitive } from '../classes/structures';
export const Dataset: Primitive[] = ${JSON.stringify(this.Primitives)}`);
    ...
    ImportNLW(Extension, Source) {
        ...
        const Primitive = {
            ...
            Precedence: 0,
            DefaultOption: Source.defaultArgCount,
        };
    ...
    ImportNL(Extension, Source) {
        ...
        const Primitive = {
            ...
            BlockContext: Source.syntax.blockAgentClassString == 'null'
