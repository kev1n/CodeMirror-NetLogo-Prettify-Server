...
import { EditorLanguage, ParseMode } from './editor-config.js';
import { highlight } from './codemirror/style-highlight.js';
import { updateExtension } from './codemirror/extension-update.js';
import { stateExtension } from './codemirror/extension-state-netlogo.js';
...
import { CompilerLinter, RuntimeLinter } from './lang/linters/runtime-linter.js';
...
export class GalapagosEditor {
    ...
    RemoveChild(Child) {
        ...
        if (Child.ParentEditor !== this)
            throw new Error('Cannot remove an editor that is not my child.');
    ...
    Detach() {
        if (this.ParentEditor == null)
            throw new Error('Cannot remove an editor that is not a child.');
    ...
    SetCompilerErrors(Errors) {
        var State = this.GetState();
        if (State.CompilerErrors.length == 0 && State.RuntimeErrors.length == 0 && Errors.length == 0)
            return;
    ...
    SetContext(context) {
        if ((this.Options.ParseMode == ParseMode.Oneline || this.Options.ParseMode == ParseMode.Reporter) &&
            ...
            if (child.Options.ParseMode == ParseMode.Normal || child == this) {
                ...
                for (var [p, vars] of preprocess.BreedVars) {
                    for (var variable of vars) {
                        ...
                        let vars = mainPreprocess.BreedVarToPlurals.has(variable)
    ...
    async ForceLintAsync() {
        ...
        for (var Extension of this.Linters) {
            var Results = await Promise.resolve(Extension.Source(this.CodeMirror));
